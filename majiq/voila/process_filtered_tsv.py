...
with open('filtered_all.tsv') as f:
    for i, line in enumerate(f):
        ...
        chrom, strand, juncoord, exon_coord, ircoord, ucsc = line.replace('\n', '').split('\t')
...
with open('sorted_all.psi.tsv', 'w') as f:
    ...
    for (geneid, lsvid, lsvtype, epsi, stdev, a5ss, a3ss, es, njunc, nexon, juncoord, ircoord,
        ...
        f.write(
            f'{geneid}\t{lsvid}\t{lsvtype}\t{epsi}\t{stdev}\t{a5ss}\t{a3ss}\t{es}\t{njunc}\t{nexon}\t{juncoord}\t{ircoord}\t{start1}\t{end1}\t{start2}\t{end2}\n')
