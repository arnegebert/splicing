...
% TODO: fix graphic inlcusion after folder changes, but possibly do some more folder changes too
...
\begin{table}[h!]
	...
	\begin{tabular}{| l | c | c | c| c} 
		...
		\hline
	\end{tabular}
	\caption{Performance on the dataset based on processing with MAJIQ and iPSC cells differentiated to neurons with and without length features given as AUC. Note that the relative performance drop was computed with reference to the baseline AUC value of 0.5.
	}
	\label{table:results_hexevent}
\end{table}

%TODO: bar chart for comparison
