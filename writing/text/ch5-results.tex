...
%TODO say somewhere that validation and test error are always the same
%TOOO also talk about training error
...
This is also surprising from a machine learning perspective, as the cerebellum tissue-based dataset contains almost twice as many training samples as the heart tissue-based dataset. This indicates that either 1) the models have already hit a point of diminishing returns for adding more data or 2) the models generally require significantly more training samples. All models perform best on the dataset based on a brain cortex sample. %TODO: not happy with the wording of this section yet; could be more dense.
...
\begin{figure}[h]
	\centering\includegraphics[width=0.9\textwidth]{../visualizations/ch5-results/gtex_junc_barcharts.png} 
...
\begin{figure}[h]
	...
	\includegraphics[width=0.5\textwidth]{../saved/log/GTEx_Junc_Heart_Attn/junc_cv_final/ROC.png}
	\caption{ROC curves when training GTEx-based intron-centric heart tissue dataset. Left: ROC curve of DSC. Right: ROC curve of RASC. }
%	TODO: make figure share y-axis; fix t=plot titles
...
Figure \ref{fig:gtex_junc_barcharts} shows that junction classification is seemingly an easier task: the AUC of all models increases by at least 0.05 to 0.06. This is likely a result of the intron-centric datasets containing on four times more training samples. RASC significantly outperforms DSC and D2V. The performance differences between tissues vary per model, however, all models tend to perform the best on the cerebellum tissue which also contains the most training samples. 

Surprisingly, the performance across rarely and highly included junctions is no longer very disparate (see Figure \ref{fig:gtex_junc_rocs}). This indicates that the models learn to recognize motifs which separate constitutive and alternatively spliced junctions, but can't do so for exons. However, this observation may also be  as only 15\%

%15 \% of alternatively spliced exons are highly included 
%
%TODO: no lens experiments (done on cerebellum): DSC + RASC missing
%TODO: run cross-validation runs of this: DSC Heart + DSC Brain missing
...
\end{figure}
%TODO: make this one figure with shared axes (finetuning)
...
\subsubsection{Impact of architectural choices}
...
As litmus test for dataset quality, we repeat the same experiment as on the HEXEvent dataset and remove the length features as model input. We document the results in Table \ref{table:majiq_nolens}.
They reveal that a large part of D2V's performance relies on information contained in the length features. However, since removing the length features only marginally affects the performance of DSC and RASC, we conclude that D2V is unable to effectively incorporate information from the sequences and that the dataset is not biased in the same way as the HEXEvent dataset. 

Giving the strong model performance independent of the length features and the prior expectation for MAJIQ to produce the highest quality PSI estimates, we conclude that it does. This gives further weight to the conclusions that the poor performance on the exon-centric GTEx-based and HipSci SUPPA datasets is a result of poor data quality, and not due to the inherent hardness of the splicing classification task. Thus, using the HipSci MAJIQ datasets is preferable to using any of the other datasets we have evaluated. 
...
\subsubsection{Sensitivity and specificity analysis}
\begin{figure}
...
\subsection{Cross-tissue performance} \label{subsec:hipsci_ipsc_majiq}
...
\begin{figure}
	\centering\includegraphics[width=1\textwidth]{../visualizations/ch5-results/attention_heatmap_plswork.png} 
	\caption{Development of attention during training. }
	\label{fig:attn_heatmap}
\end{figure}

We visualize to what parts of the input sequence RASC pays attention to in \ref{fig:attn_distribution}. %TODO
...
%TODO: probably want to put into respective focus into table actually -- or by redoing heatmap such that you observe behavior there too (aka actual attention over sequence), might want to switch color tbh
...
\begin{figure}
	\centering\includegraphics[width=1\textwidth]{../visualizations/ch5-results/mean_attention_w_std.png} 
	\caption{Mean attention weights averaged over all test samples and all four attention heads. }
	\label{fig:mean_attn}
\end{figure}

%- in contrast to the single attention head case, with multiple attention heads they focus on solely on the start or end sequence\\
%- decision where an attention head focuses is made early on in training - within the first epoch even(?). similar as observation from, do you really need 16 attention heads paper, where attention heads become more or less important as time goes on\\
%-> show attention distribution across one model\\
%- attention heads split that at least one head focuses on either sequence\\
%- worst run is where attention heads purely focus on end sequence (?) -> no, never happens


% how do I best visualize my findings?
% - I want to showcase all attention heads at some point
...
%
%---> get one good run where all heads focus on the sma esequence 
%---> one run where they don't
%- hopefully both runs are long (150 epochs+)
