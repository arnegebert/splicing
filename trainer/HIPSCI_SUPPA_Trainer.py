...
class HIPSCI_SUPPA_Trainer(BaseTrainer):
    """
    Trainer class
    """
    def __init__(self):
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, data_all in enumerate(self.val_all):
                seqs = data_all[:, :280].view(-1, 2, 140, 4)
                lens, target = data_all[:, 280, :3], data_all[:, 280, 3]
            ...
            for batch_idx, data_low in enumerate(self.val_low):
                seqs = data_low[:, :280].view(-1, 2, 140, 4)
                lens, target = data_low[:, 280, :3], data_low[:, 280, 3]
            ...
            for batch_idx, data_high in enumerate(self.val_high):
                seqs = data_high[:, :280].view(-1, 2, 140, 4)
                lens, target = data_high[:, 280, :3], data_high[:, 280, 3]
