...
class HIPSCI_MAJIQ_DataLoader(BaseDataLoader):
    """
    ...
    def __init__(self, data_dir, batch_size, shuffle=True, validation_split=0.0, num_workers=1, training=True,
        ...
        if data_dir:
            ...
            hx_cas_data = np.load(f'{data_dir}/high2.npy')
            lx_cas_data = np.load(f'{data_dir}/low2.npy')
        else:
            x_cons_data = np.load('data/hipsci_majiq/junc/cons.npy')
            hx_cas_data = np.load('data/hipsci_majiq/junc/high2.npy')
            lx_cas_data = np.load('data/hipsci_majiq/junc/low2.npy')
        ...
        train_dataset = HIPSCI_MAJIQ_Dataset(train)
        val_all_dataset = HIPSCI_MAJIQ_Dataset(val_all)
        val_low_dataset = HIPSCI_MAJIQ_Dataset(val_low)
        val_high_dataset = HIPSCI_MAJIQ_Dataset(val_high)
...
class HIPSCI_MAJIQ_Dataset(Dataset):
